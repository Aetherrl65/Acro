...
folder_pathh = "data/checkpoints/acrolatest-1715315591392413400/20656898"
class Learner(object):
    def __init__(
            ...
            timestep_limit: int = 10e60,
            exp_buffer_size: int = 100000,
            ts_per_iteration: int = 200000,
            ...
            policy_layer_sizes: Tuple[int, ...] = (512, 512, 512),
            critic_layer_sizes: Tuple[int, ...] = (512, 512, 512),
            continuous_var_range: Tuple[float, ...] = (0.1, 1.0),

            ppo_epochs: int = 2,
            ppo_batch_size: int = 200000,
            ...
            checkpoints_save_folder: Union[str, None] = folder_pathh,
            add_unix_timestamp: bool = True,
            checkpoint_load_folder: Union[str, None] = folder_pathh,
        ...
        # Initialize checkpoint_load_folder with the last saved folder if not specified
        #if checkpoint_load_folder is None:
            # Check if the specified checkpoints_save_folder exists
            #if os.path.exists(checkpoints_save_folder):
                # Find the last saved folder in checkpoints_save_folder
                #saved_folders = [
                    #folder for folder in os.listdir(checkpoints_save_folder)
                    #if os.path.isdir(os.path.join(checkpoints_save_folder, folder))
                #]
                #if saved_folders:
                    #last_saved_folder = max(saved_folders)
                    #checkpoint_load_folder = os.path.join(checkpoints_save_folder, last_saved_folder)
                #else:
                    # Handle case where no saved folders are found
                    #print("No checkpoint folders found in", checkpoints_save_folder)
                    # You can provide a default behavior here or raise an error
                    # For example:
                    # raise FileNotFoundError("No checkpoint folders found")
            #else:
                # Handle case where checkpoints_save_folder does not exist
                #print("Specified checkpoints_save_folder does not exist:", checkpoints_save_folder)
                # You can provide a default behavior here or raise an error
                # For example:
                # raise FileNotFoundError("Specified checkpoints_save_folder does not exist")

...
class VelocityReward2(RewardFunction):
    # ...

    def get_reward(self, player: PlayerData, state: GameState, previous_action: np.ndarray) -> float:
        velocity_reward = np.linalg.norm(player.car_data.linear_velocity) / CAR_MAX_SPEED * (1 - 2 * self.negative)
        
        # Calculate distance between player and ball
        distance_to_ball = np.linalg.norm(player.car_data.position - state.ball.position)
        
        # If player is close to ball and going slow, penalize
        if distance_to_ball < CLOSE_TO_BALL_THRESHOLD and velocity_reward < SLOW_VELOCITY_THRESHOLD:
            velocity_reward *= -1  # Make reward negative
